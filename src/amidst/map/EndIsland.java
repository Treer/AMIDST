...
import amidst.minecraft.Simplex2dNoise;
...
public class EndIsland {
	...
	private static Simplex2dNoise _cachedNoiseFunction = null;
	private static long _cachedNoiseFunctionSeed;
	
	/** Returns the noise function using the current seed */
	protected static Simplex2dNoise NoiseFunction() {
		...
		if (_cachedNoiseFunctionSeed != Options.instance.seed || _cachedNoiseFunction == null) {
			...
			Perlin3dOctaves_FakeConstructor(random, 16);			
			_cachedNoiseFunction = new Simplex2dNoise(random);
		}
	...
	protected static EndIsland checkForIsland(int chunkX, int chunkZ, Simplex2dNoise noiseFunction) {
		...
		if (chunkX == 0 && chunkZ == 0) {
			...
			if (noiseFunction.a(chunkX, chunkZ) < cIslandDensityThreshold) {
	...
	public static List<EndIsland> findSurroundingIslands(int chunkX, int chunkY, int chunksWide, int chunksDeep) {
		...
		Simplex2dNoise noiseFunction = NoiseFunction();
